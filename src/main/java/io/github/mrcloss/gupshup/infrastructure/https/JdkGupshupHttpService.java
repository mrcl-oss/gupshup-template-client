...
public class JdkGupshupHttpService implements GupshupHttpService {
    ...
    @Override
    public <T extends BaseGupshupResponse> T postForm(String path, Map<String, Object> body, Class<T> responseType) {
        try {
    ...
    public <T extends BaseGupshupResponse> CompletableFuture<T> postFormAsync(String path, Map<String, Object> body, Class<T> responseType) {
        try {
            ...
            return CompletableFuture.failedFuture(new GupshupApiException("Failed to start async POST request: " + e.getMessage(), 0, null));
        }
    }
    ...
    @Override
    public <T extends BaseGupshupResponse> CompletableFuture<T> deleteAsync(String path, Class<T> responseType) {
        try {
            HttpRequest request = buildBaseRequest(path)
                    .DELETE()
                    .build();
    ...
    private String buildFormBody(Map<String, Object> body) {
        return body.entrySet().stream()
                ...
                .map(e -> {
                    ...
                    return URLEncoder.encode(e.getKey(), StandardCharsets.UTF_8) + "=" + URLEncoder.encode(value, StandardCharsets.UTF_8);
                })
    ...
    private <T extends BaseGupshupResponse> T handleResponse(HttpResponse<String> response, Class<T> responseType) {
        ...
        try {
            ...
            throw new GupshupApiException("Failed to parse Gupshup response: " + e.getMessage(), statusCode, body);
        }
    }
}
