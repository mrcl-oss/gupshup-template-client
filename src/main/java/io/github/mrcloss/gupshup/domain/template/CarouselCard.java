...
public class CarouselCard {
  ...
  public CarouselCard(
      String body, List<String> variableExamples, CarouselCardHeaderType headerType) {
    this.body = body;
  ...
  public void validate() {
    ...
    if (hasVariablesInBody && !hasExamples) {
      throw new IllegalStateException(
          "Carousel card body cannot contain variables if variable examples are not provided");
    }
    ...
    if (hasExamples) {
      for (int i = 1; i <= variableExamples.size(); i++) {
        ...
        if (!body.contains(placeholder)) {
          throw new IllegalStateException(
              "Carousel card body must contain "
                  + placeholder
                  + " when "
