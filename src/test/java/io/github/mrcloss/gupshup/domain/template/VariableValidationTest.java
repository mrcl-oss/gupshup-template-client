...
public class VariableValidationTest {
    ...
    void templateShouldFailIfBodyHasVariablesButNoExamples() {
        ...
        assertThrows(IllegalStateException.class, template::validate, "Should fail if body has variables but no examples");
    }
    ...
    void textTemplateShouldFailIfHeaderHasVariablesButNoExamples() {
        ...
        assertThrows(IllegalStateException.class, template::validate, "Should fail if header has variables but no examples");
    }
    ...
    void carouselCardShouldFailIfBodyHasVariablesButNoExamples() {
        CarouselCard card = new CarouselCard("Hello {{1}}!", null, CarouselCard.CarouselCardHeaderType.IMAGE);
        // carousel card needs buttons to be valid
        card.setButtons(Collections.singletonList(new io.github.mrcloss.gupshup.domain.button.QuickReplyButton("Yes")));
        
        assertThrows(IllegalStateException.class, card::validate, "Should fail if carousel card body has variables but no examples");
    }
}
