...
class GupshupClientTest {
    ...
    @BeforeEach
    void setUp() {
        client = DefaultGupshupClient.builder()
                .appId(appId)
                .apiKey(apiKey)
    ...
    void testCreateTemplate() {
        ...
        when(httpService.postForm(anyString(), anyMap(), eq(CreateTemplateResponse.class))).thenReturn(expectedResponse);
        ...
        verify(httpService).postForm(eq(""), anyMap(), eq(CreateTemplateResponse.class));
    }
    ...
    void testDeleteTemplate() {
        ...
        when(httpService.delete(anyString(), eq(DeleteTemplateResponse.class))).thenReturn(expectedResponse);
        ...
        // Assert
        assertEquals("success", response.getStatus());
        verify(httpService).delete(eq("/" + templateName), eq(DeleteTemplateResponse.class));
    }
    ...
    void testDeleteTemplateAsync() throws Exception {
        ...
        when(httpService.deleteAsync(anyString(), eq(DeleteTemplateResponse.class))).thenReturn(CompletableFuture.completedFuture(expectedResponse));
        ...
        // Assert
        assertEquals("success", response.getStatus());
        verify(httpService).deleteAsync(eq("/" + templateName), eq(DeleteTemplateResponse.class));
    }
}
